...
\paragraph{Main theorem.} \emph{Let $\sigma$ be a primitive Pisot substitution on a finite alphabet whose incidence matrix has irreducible characteristic polynomial, and suppose its balanced-pair automaton $\B_\sigma$ is finite \textup{(G1)}. Then, conditional on the SCC Producer Theorem (Conjecture~\ref{conj:producer}), the associated tiling dynamical system has \PDS.} Equivalently,
\[
...
Neither use enters the contracting conjugate space. No fractal is constructed; no algebraic conjugate of $\beta$ is named. The contracting conjugates appear only inside the Moss\'e recognizability radius $R(\sigma)$, which we use as a black box.

This is not a claim that \PSC\ can be proved without Pisot geometry: $\beta>1$ is essential, and the Pisot character of $\beta$ enters indirectly through Moss\'e recognizability. The narrower claim is that the proof avoids case-by-case estimates in the contracting conjugate space, and so avoids the explicit conjugate arithmetic that becomes painful for large $|\A|$.
...
\item \emph{aperiodic}: no fixed point of $\sigma$ is eventually periodic (automatic for primitive substitutions on $|\A|\ge 2$ letters~\cite{Queffelec});
...
\begin{itemize}[leftmargin=*]
\item Barge--Diamond~\cite{BD} prove that for some letter pair $a\ne b$, $\sigma^n(a)$ and $\sigma^n(b)$ share a letter at the same position with matching prefix Parikhs at some $n\ge 0$. This gives productivity for one specific seed but does not propagate to arbitrary recurrent SCCs.
\item Hollander--Solomyak~\cite{HS} extend BD to all letter pairs in the two-letter case, settling Conjecture~\ref{conj:producer} for $|\A|=2$. This is the only alphabet size in which the conjecture is fully established.
...
Conditional on G1 (Hypothesis~\ref{thm:finiteness}, finiteness of $\B_\sigma$) and on Conjecture~\ref{conj:producer}, every primitive Pisot substitution with irreducible characteristic polynomial gives rise to a tiling dynamical system with \PDS.
\end{theorem}
...
Let $\sigma$ be a primitive Pisot substitution on a finite alphabet whose incidence matrix has irreducible characteristic polynomial, and suppose $\B_\sigma$ is finite \textup{(Hypothesis~\ref{thm:finiteness}, G1)}. Then, conditional on the SCC Producer Theorem (Conjecture~\ref{conj:producer}), the associated tiling dynamical system has \PDS.
\end{theorem}
...
\item The SCC Producer Theorem (Conjecture~\ref{conj:producer}). This is settled for $|\A|=2$ by Hollander--Solomyak~\cite{HS}, partially by Barge--Diamond~\cite{BD} (one seed), and unconditionally for $\beta$-substitutions and certain self-similar families by~\cite{Barge2018,AL}; the general case is open. The mass-balance and algebraic-structure lemmas (\S\S\ref{sec:mass-balance}--6.3) reformulate the open content but do not close it. For $|\A|=3$, Proposition~\ref{thm:v34} contributes a spectral lower bound on the within-SCC matrix that is informative but, by Remark~\ref{rem:v34-scope}, insufficient to close the conjecture even on alphabet 3.
\end{enumerate}
...
\item Explicit padding bound via the phase automaton (Observation~\ref{prop:padding}): $D(\sigma)\le|\A|^2|\sigma|_{\max}^2$, replacing the unspecified ``padding bound'' constant in earlier formulations.
