...
def test_named_representatives_pin_selector_phases() raises:
    ...
    # C: good {0,1}, hub 2. Both viable star pairs flow with residual 0.
    ...
    # D: every bad edge is fixed; phase 0 for every good-edge placement.
    ...
    # E: the phase depends on which edge is Barge-Diamond-good.
    ...
    # F: viable regimes have phase 1; one good-edge placement kills all orbits.
...
def test_all_cdef_maps_have_uniform_viable_phase() raises:
    ...
    # Exact conjugacy-expanded classification counts over (map, good edge).
...
def test_viability_rejects_good_edge_and_coalescence() raises:
    ...
    # Under C, {0,1} coalesces immediately when the good edge is elsewhere.
