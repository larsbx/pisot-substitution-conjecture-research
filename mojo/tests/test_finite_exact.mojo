"""Executes the vendored finite_exact smoke checks under the PSC toolchain.

The byte-level identity of the vendored files with the pinned upstream is
checked separately by scripts/check_finite_exact_sync.py.
"""
...
from finite_exact.interval_q import bigq_interval_conformance_smoke, demo_complex_quadrance_point, demo_interval_mul
...
def main() raises:
    ...
    if not bigq_interval_conformance_smoke() or not demo_interval_mul() or not demo_complex_quadrance_point():
        raise Error("finite_exact IQ smoke failed")
