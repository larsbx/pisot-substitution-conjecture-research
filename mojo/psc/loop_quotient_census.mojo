...
Two exact relations are available. The first uses one nominated digit pair on
both sides, preserving the original `(1,2)` regression. The second admits any
one-level extension in which top and bottom each insert one digit pair at the
same ancestry position; the two inserted side-digits need not agree. This tests
whether residual collisions are still explained by finite symbolic recurrence
before any arithmetic/residue coordinate is introduced.
...
struct LoopQuotientAudit(Copyable, Movable):
    """Collision counts before and after one observed-extension quotient."""
...
def audit_projection_modulo_loop(
    ...
    return _audit_projection_with_relation(
        samples, 0, parent_letter, child_index
    )
...
def audit_projection_modulo_synchronous_extensions(
    ...
    Projection classes are interned in near-linear expected time. Pairwise edge
    tests occur only inside actual projection-collision classes, so the secondary
    work scales with the collision population rather than the square of the full
    corpus.
    """
    return _audit_projection_with_relation(samples, 1, 0, 0)
