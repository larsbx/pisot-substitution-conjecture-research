...
def test_commented_document_sentinels_fail(copy):
    ...
    allow(copy, "newif ifdraft else fi iffalse iff ifthenelse inputencoding csnamex begingroup endgroup foo renewcommand newenvironment "
                "comment "
                "DeclareMathOperator Tr DeclareGraphicsExtensions newcounter newlength len newdimen dim bar baz qux providecommand")
    ...
    tex.write_text(text.replace("\\begin{document}", "\\DeclareMathOperator{\\Tr}{Tr}\\newcounter{foo}\\newlength{\\len}\\newdimen\\dim\n"
                                "\\newcommand\\foo{}\\newcommand*{ \\qux }[1]{#1}\\providecommand{\\baz}{\\qux{x}}\n\\begin{document}", 1))
    assert run(copy)[0] == 0  # Declare... macros, allocators and ...command... definers naming fresh targets (\\bar is a kernel word)
