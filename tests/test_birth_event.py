...
def test_birth_events_retain_orientation_and_endpoint_data():
...
def test_every_deep_newborn_boundary_is_an_occurrence_of_one_step_type():
    tau = strict_derived_substitution(SIGMA, COMP)
    catalog = strict_birth_event_catalog(SIGMA, COMP)
    occurrences = newborn_occurrences(tau, A, depth=3, catalog=catalog)
    assert tuple(item.derived_boundary_index for item in occurrences) == (1, 3, 5, 7)
    assert tuple(item.event.key for item in occurrences) == (
        (A, 1),
        (B, 1),
        (A, 1),
        (B, 1),
...
def test_missing_birth_event_type_is_rejected():
    ...
    catalog = strict_birth_event_catalog(SIGMA, COMP)
    try:
        newborn_occurrences(tau, A, depth=2, catalog=catalog[:1])
