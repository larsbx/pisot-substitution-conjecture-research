...
**Status:** this repository's binding table for the rational and interval arithmetic specification in `larsbx/finite-math-kernels`. It states no theorem: exactness removes one class of error, and the epistemic status of every computation is governed by `verification-architecture.md` and the claim-status documents.
...
| --- | --- | --- | --- |
| 1.1 integer backend, 1.1–1.3 ℚ | `mojo/finite_exact/bigint_z.mojo`, `mojo/finite_exact/rational.mojo` | CONFORMS | vendored from the one monorepo commit pinned in `vendored.toml`; per-file SHA-256 drift fails CI |
| 2.1–2.5 I_Q and rank-2 boxes | `mojo/finite_exact/closed_interval.mojo` | CONFORMS | same repository, commit, and digest discipline |
...
| 1–2 direct consumers | `mojo/finite_linear_algebra/{matrix,rational_elimination,tensor3,w3}.mojo`, `mojo/psc/pisot.mojo`, `mojo/psc/real_root_sign.mojo` | CONFORMS | exact linear algebra, Sturm sequences, and the PIP screen over unbounded rationals; the former machine-width `Rat` and `CheckedRat` are deleted |
...
Hook (specification section 7, consumer side): the vendoring pin in `vendored.toml` checked in CI and by `scripts/verify_all.sh`; the law and smoke checks of the packages re-run under the PSC toolchain by `mojo/tests/test_finite_exact.mojo`; the PSC-specific interval tests in `mojo/tests/test_exact_interval.mojo` (natural Horner extension containment, division across zero fails closed, Perron-root enclosure), which are the first external consumer test named by the `interval_q` package. The floating-point ban of criterion C1 is enforced by the `no-float-in-kernel` rule of `claim_governance.toml` (vendored `tools/claim_governance`, run in CI, `pytest`, and `verify_all.sh`) over `mojo/psc/`, the census and certificate programs, and the Mojo tests; `audit-2026-09-15.md` recorded the same fact by inspection before the rule existed.
