...
| B7 shared real times 2-adic box kernel | carrier delivered 2026-09-17 (`FMK: bd68d11`), though not in B7's own terms: its literature gate refuted the scalar reading, since one `Z_p` ball per rational prime cannot model `Z^3 / M^k Z^3`, which is `Z/2 x Z/2` at `k = 2` where a precision-two `Z_2` ball is cyclic. `FMK: finite_linear_algebra/madic_ball.mojo` carries cosets of `M^k Z^n` instead, and certifies separation only; `PSC: mojo/psc/finite_cokernel_address.mojo` still computes the classes | R7, for the theorem step a carrier does not supply. The local-field factor over `Q(beta)` stays unbuilt on purpose |
...
| R7 | partly delivered, two of the four ranked parts. The degree-`n` Pisot screen, canonical in `PSC: mojo/psc/pisot_screen.mojo`, behind a literature gate that narrows the claim to an exact fail-closed implementation of Schur–Cohn composed with the bilinear transform and Routh–Hurwitz, and that records one capability bound: `x^3 - 3x^2 - 3x - 3` is a Pisot polynomial this method refuses. And the **carrier** of the p-adic part, which its own gate licensed only after refuting the shape B7 asked for. This cell records a carrier rather than a result, and the distinction is the gate's: an M-adic ball certifies that two points differ and never that they agree, so the theorem step it serves is untouched by its existence | `PSC: 31effdb`, from `e398958`, `da2eca4`, `1a254d3`, `426838f`; `FMK: bd68d11`, from `9241cc9`, `41a2fec` | two ranked parts: the Lean lemma for irreducibility of the exterior-square characteristic polynomial, and the Hubbard-tree core-entropy census. Both untouched. The Lean lemma's obstacle is environmental rather than mathematical: no Lean toolchain is available here, so a proof could only be checked by pushing it to CI. Owed by the programme rather than by R7's ranked definition: the p-adic part's theorem step, which stays behind the splicing and tiling-dictionary bridge. The ranked item asked for a module and a module is what shipped, so this row is closed on the p-adic part while the mathematics it serves is not |
| R8 | not started | | |
