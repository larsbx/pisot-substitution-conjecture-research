...
| R7 | partly delivered. The degree-`n` Pisot screen, canonical in `PSC: mojo/psc/pisot_screen.mojo`, behind a literature gate that narrows the claim to an exact fail-closed implementation of Schur–Cohn composed with the bilinear transform and Routh–Hurwitz, and that records one capability bound: `x^3 - 3x^2 - 3x - 3` is a Pisot polynomial this method refuses | `PSC: e398958`, `PSC: da2eca4`, `PSC: 1a254d3`, `PSC: 426838f` | three of the four ranked parts. The Lean lemma for irreducibility of the exterior-square characteristic polynomial, the Hubbard-tree core-entropy census, and the p-adic module are untouched. The Lean lemma's obstacle is environmental rather than mathematical: no Lean toolchain is available here, so a proof could only be checked by pushing it to CI |
| R8 | not started | | |
