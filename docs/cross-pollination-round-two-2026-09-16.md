...
| B8 Hubbard-tree core-entropy census | not done. **Post-audit, 2026-09-17:** its literature gate landed (`NLAP: da30443`) and licenses the census, so what is missing is the implementation, not the permission | R7 |
...
The core-entropy gate supplied the last four, and two of them inverted a conclusion rather than softening one. It had cited Thurston's realisation theorem to close the degree-two image question; that theorem is about multimodal maps of unrestricted degree, and the literature describes restricted degree as exactly where the question becomes subtle, so the note had forbidden a question that is open. It had also declared implementation unlicensed until the Pisot screen was extracted to the kernels monorepo, when this program's own extraction plan puts Pisot tests out of the shared library's scope and keeps the predicates where they are: extraction is the one route the plan rules out, so that blocker would have held R7 on a policy that does not exist. The remaining two: its headline narrowed the census population to the primitive subfamily while its own finding said to partition and warned that filtering creates a selection effect, and its known-list cross-check was written without the scope under which it holds, since the Dufresnoy–Pisot enumeration is explicit only below the golden ratio. All four were caught in a stop/go note, before any code existed for them to be wrong about.
