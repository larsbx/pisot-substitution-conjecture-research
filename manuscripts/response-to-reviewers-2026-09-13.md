...
| 23 | P2 | The probe passed the heuristic window `14 + k` to `oa_types`, below the documented bound `(g(W) + l_max)/l_min` for large length ratios, so level-0 types could be missed | Accepted. The bound is now computed exactly in `Q(β)` (`oa_window`: least `n` with `n·l_min > g(W) + l_max`, decided by exact signs) and used by default; an explicit smaller window, or a prefix too short for the window, raises (fail closed). The same heuristic (`window = 14`, `k ≤ 8`) had been used for the Section 7 sample table, so both the sample and the probe were re-run with the exact window and the reported data replaced | `src/psc_research/oa_overlap_graph.py` (`oa_window`, `oa_types`, `type_inclusion_report`), both scripts, regression tests in `tests/test_oa_overlap_graph.py`; note Section 7 table and addendum |

### Finding 23 (P2)

Accepted. The referee's example is exact: for `1 → 2, 2 → 3, 3 → 133` the bound at `k = 24` is 87 against the 38 passed. Since a too-small window can only omit level-0 types, the earlier figures could over-report inclusion; they are withdrawn and replaced by the exact-window rerun, still labelled exploratory and uncertified.
