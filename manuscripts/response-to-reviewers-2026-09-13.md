...
## Seventy-ninth round (pull request #95, primitive-stop revision)

One finding from the automated Codex review of commit `c5fc74d300`; accepted, on the fail-closed side.

| # | Priority | Finding (short) | Action | Where in the revision |
| --- | --- | --- | --- | --- |
| 143 | P2 | A package's name-based definer (`\csdef{begin}{}`) evaded every definer rule | Accepted, and resolved by an allowlist rather than by another pattern, since the set of package macros that define a control sequence from a name is open-ended. `manuscripts/TEX_CONTROL_WORDS` (listed in the manifest) now names the control words a source may use, one per line, sorted; every control word before the closing sentinel must be listed, or the source fails, and the list itself must be sorted, without repetition, nonempty, made of names, and free of the control words the guard can never follow. The list was generated from the two repository manuscripts (167 words, all standard LaTeX and amsmath control words plus the manuscripts' own macros; the only definers are `\newcommand` and `\newtheorem`). Control symbols (a backslash and one non-letter) define nothing and are exempt. Four further gaps were closed alongside, each confirmed to pass the previous revision: a macro body may not reorder its parameters (`#2#1` would move a definer next to a sentinel word in ways the guard cannot follow; parameters must be `#1`..`#9`, inside a body, strictly increasing within one top-level group), a `...command...` definer must be followed by its body or argument count after its control-word target (`\newcommand\foo\bar` fails), an environment definer may not target `begin`, `end` or `enddocument` either, and the manuscripts directory may hold no TeX input file (`.sty`, `.cls`, `.clo`, `.def`, `.cfg`, `.fd`, `.ltx`, `.dtx`, `.ins`) that a source could load in place of the installed one. The reported scenario was confirmed to pass the previous revision. Tests: `\csdef{begin}{}` with `\csdef{end}{}`, `\cslet{end}\relax`, `\@namedef{beginfoo}{}` and an unlisted `\newcommand{\foo}{x}` (all fail as unlisted), the same `\foo` once listed and `\csdef` after `\end{document}` (pass), a list containing `\def`, an unsorted, repeated, malformed or empty list, and a missing list (all fail); `#2#1`, `#1#2#1`, `\f{#2}{#1}`, a `#1` outside any body, `##1` and `#a` (all fail) and `\f{#1}{#2}` with a one-parameter `\f` (passes); `\newcommand\foo\bar`, `\NewCommandCopy\foo\bar`, `\renewenvironment{begin}{}{}` and `\newenvironment*{ end }{}{}` (all fail); `geometry.sty`, `article.cls` and `t1enc.def` in the directory (all fail) | `scripts/check_manuscript_source.py`, `manuscripts/TEX_CONTROL_WORDS`, `manuscripts/MANIFEST`, `tests/test_check_manuscript_source.py` |

## Eightieth round (pull request #95, allowlist revision)

One finding from the automated Codex review of commit `e22297f7b7`; accepted, on the fail-closed side.

| # | Priority | Finding (short) | Action | Where in the revision |
| --- | --- | --- | --- | --- |
| 144 | P2 | A sentinel wrapped in a macro body and invoked early was ignored rather than rejected | Accepted. Every `\begin{document}` or `\end{document}` before the closing sentinel other than the two standalone sentinels themselves now fails the source, since a macro or group could execute it; and `\begin`/`\end` must be followed by `{name}` with a plain name (letters, digits, `*`), since a name assembled from a macro parameter (`\begin{docu#1}`, `\end{#1}`) could spell `document`. The reported scenario and the two assembled-name forms were confirmed to pass the previous revision. Tests: `\newcommand{\foo}{\end{document}}` then `\foo`, `\newcommand{\foo}{\begin{document}}` and `x \end{document}` before the sentinels (all fail as not the standalone sentinel); `\begin{docu#1}`, `\end{#1}` in macro bodies and `\begin{ center }` (all fail as sentinel tampering); `\begin{align*}` (passes) | `scripts/check_manuscript_source.py`, `tests/test_check_manuscript_source.py` |

## Eighty-first round (pull request #95, stray-sentinel revision)

One finding from the automated Codex review of commit `f1e1ee8fb3`; accepted, on the fail-closed side.
...
## Eighty-seventh round (pull request #95, literal-name revision)

One finding from the automated Codex review of commit `747fd3b77f`; accepted, on the fail-closed side.

| # | Priority | Finding (short) | Action | Where in the revision |
| --- | --- | --- | --- | --- |
| 152 | P2 | Sentinels wrapped in a `comment` environment sat at brace and conditional depth zero and passed | Accepted. The environment uses up to the closing sentinel must now nest properly (at any brace depth, since a macro body cannot be told apart), with no environment open at the opening sentinel and nothing but `document` open at the closing one: a sentinel inside any other environment, an environment left open at either sentinel, an `\end` closing no open environment or closing a different one, and a macro body containing an unmatched `\begin` all fail the source. The scenario was confirmed to pass the previous revision. Tests: the sentinels inside `comment`, `\begin{center}` left open before the opening sentinel and before the closing one, a stray `\end{center}`, `\begin{center}\end{quote}` and `\newcommand{\foo}{\begin{center}}` (all fail); a macro body holding a whole environment and an `\end{center}` after `\end{document}` (pass) | `scripts/check_manuscript_source.py`, `tests/test_check_manuscript_source.py` |

## Eighty-eighth round (pull request #95, environment-nesting revision)

One finding from the automated Codex review of commit `385bc8e3c0`; accepted, on the fail-closed side.

| # | Priority | Finding (short) | Action | Where in the revision |
| --- | --- | --- | --- | --- |
| 153 | P2 | A bare argument-taking command before the opening sentinel would take `\begin` as its argument | Accepted. The guard now carries the mandatory-argument count of every allowlisted control word (and of every macro a source declares with `\newcommand`, from its declaration), and checks the up to nine argument units (brace groups, control words, control symbols, single characters) before each sentinel and before the end of every brace group other than a definer's target group `{\name}`: a call among them with fewer units after it than it takes (a `*` and `[...]` groups right after the call being optional; a declared `[n][default]` makes the first argument optional), or one whose arity the guard does not know, fails the source, since a macro body's expansion, an argument a body re-emits and a bare macro passed as a single-token argument are all followed by whatever comes after the call, the sentinel included. The bare `\title` scenario, a bare `\emph` before the closing sentinel and a macro body ending in `\emph` invoked before the opening sentinel were all confirmed to pass the previous revision. Tests: those three, `\usepackage[a]{b}\sqrt` before the opening sentinel, a two-argument macro called with one, `\newcommand{\foo}{\frac{a}}`, `\newenvironment{foo}{x}{\emph}`, an unknown-arity word, `\foo{\sqrt}` and `\foo\sqrt` for a macro re-emitting its argument, and `\textbf{\emph}` (all fail); `\title{X}`, `\usepackage[a]{b}`, `\frac12`, `\bar\beta`, `\sqrt[3]{x}`, `\\`, `\{\}`, a body `{\emph{x}}` and a macro declared `[1][d]` called bare (pass) | `scripts/check_manuscript_source.py`, `tests/test_check_manuscript_source.py` |

## Eighty-ninth round (pull request #95, argument-boundary revision)

One finding from the automated Codex review of commit `f2d4a86492`; accepted, on the fail-closed side.

| # | Priority | Finding (short) | Action | Where in the revision |
| --- | --- | --- | --- | --- |
| 154 | P2 | A declaration anywhere in the source, including after the document or in a skipped branch, set the arity of earlier calls | Accepted. A macro now takes the arity of the latest declaration that executes before the call: one at brace depth zero, outside every conditional, before the closing sentinel and earlier in the source than the call; a conditional declared by `\newif` likewise takes none only from its declaration on. A call with no such declaration and no kernel arity has an unknown arity and fails within reach of a sentinel or a group end. A declaration later in the preamble, one in a skipped branch and one inside a group were each confirmed to establish the arity under the previous revision. Tests: `\foo` before its declaration, a declaration inside `\iffalse`…`\fi`, one inside a brace group, and one after `\end{document}` (all fail) | `scripts/check_manuscript_source.py`, `tests/test_check_manuscript_source.py` |

## Ninetieth round (pull request #95, declaration-order revision)

One finding from the automated Codex review of commit `0eccfebc91`; accepted, on the fail-closed side.

| # | Priority | Finding (short) | Action | Where in the revision |
| --- | --- | --- | --- | --- |
| 155 | P2 | A `\providecommand` of an already defined name was recorded as a new arity although LaTeX leaves the definition unchanged | Accepted, and extended to the two neighbouring cases: only a declaration LaTeX carries out now counts. `\newcommand` and `\providecommand` establish an arity only for a name not yet defined (a kernel word or an earlier executed declaration leaves them without effect, the former with an error), and `\renewcommand` only for a name that is defined; a call whose only declarations were of no effect keeps its earlier arity, or has none and fails within reach of a sentinel or a group end. The scenario, `\newcommand{\emph}{}` followed by a bare `\emph`, and `\renewcommand` of an undefined name followed by a bare call were all confirmed to pass the previous revision. Tests: those three (all fail); `\providecommand` of a new name followed by `\renewcommand*` with `[1][d]` and a bare call (passes). The four declaration-order tests announced in the eighty-ninth-round response had not reached that revision's test file, the edit that carried them having failed alongside a rejected code edit; they are committed with this revision | `scripts/check_manuscript_source.py`, `tests/test_check_manuscript_source.py` |

## Ninety-first round (pull request #95, carried-out-declaration revision)

One finding from the automated Codex review of commit `692ecf821b`; accepted, on the fail-closed side.
